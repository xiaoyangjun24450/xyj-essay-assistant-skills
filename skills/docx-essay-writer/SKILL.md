...
6. **参考文献真实引用** — 如果 chunk 中包含参考文献标记（如 `[1]`、`[2]` 或 "参考文献" 章节），不得自行编造文献信息，必须通过阶段 2.5 搜索获取真实文献
...
**触发条件：** 重写后的 chunks 中包含参考文献相关内容（如 "参考文献" 章节标题、引用标记 `[1]`、`[2]` 等）。

**执行步骤：**

1. 首先分析 `$WORK_DIR/chunks_new/` 中哪些 chunk 包含参考文献内容

2. 对于每个需要参考文献的主题，运行搜索脚本：
```bash
python3 "$SKILL_DIR/scripts/search_paper.py" \
    --query "你的参考文献主题关键词" \
    --min-results 5 \
    --output "$WORK_DIR/references_search_results.txt"
```

3. 将搜索到的真实文献信息填入参考文献位置，**禁止编造任何文献信息**

**输出：**
- `$WORK_DIR/references_search_results.txt` — 搜索到的真实参考文献列表（GB/T 7714-2015 格式）

**重要提醒：**
- 如果搜索结果为空，明确告知用户"未找到相关文献，该处无法添加真实引用"
- 引用数量应与原文保持一致
- 作者名、期刊名、年份、页码等必须来自 API 返回的真实数据
...
| 阶段 2.5 搜索结果为空 | 告知用户"未找到相关文献，该处无法添加真实引用"，保留引用标记位置但不填入内容 |
...
└── references_search_results.txt  # 真实参考文献（阶段2.5生成）
```
