...
```
🔒 阶段2B锁定已激活
允许工具：replace_in_file ONLY
禁止工具：write_to_file, ipython, execute_command, create_file
操作粒度：1个标记 = 1次replace_in_file调用
预期调用量：200+次（格式保真的必要成本，非优化目标）
```

### 执行步骤（严格顺序）
1. **读取**：找到下一个待替换的`<!--...-->`标记
2. **替换**：使用`replace_in_file`替换该**单个标记**后的文本
3. **确认**：标记部分（`<!--...-->`）必须完全保留，未被修改
4. **循环**：立即执行下一个，不询问、不中断、不优化

### 绝对禁止（违反即失败）
- ❌ 编写Python/Bash脚本批量处理
- ❌ 使用`write_to_file`重建整个文件
- ❌ 使用`ipython`进行文本处理
- ❌ 询问"是否可以优化/批量处理/使用脚本"
- ❌ 因调用次数多而暂停等待用户确认
